export function isInsideTrigon(pt, vert) {
  ...
  let cx = vert[4];
  let cy = vert[5];
  let s_ab = (bx - ax) * as_y - (by - ay) * as_x > 0;
...
export function pointToPointDistance(pt1, pt2) {
  let delx = Math.abs(pt1[0] - pt2[0]);
  let dely = Math.abs(pt1[1] - pt2[1]);
  return Math.sqrt(delx * delx + dely * dely);
}
export function pointToLineDistance(pt, line) {
  let x = pt[0];
  let y = pt[1];
  let x1 = line[0];
  let y1 = line[1];
